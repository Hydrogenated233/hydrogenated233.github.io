    ...
    addLayer("em", {
        ...
        upgrades: {
            ...
            14: {
                title: "快没了",
                effect() {
                    return player[this.layer].points.pow(1.2)
                },
                effectDisplay() { return format(upgradeEffect(this.layer, this.id)) + "x" }, // Add formatting to the effect
                description: "电磁力加成更多光的獲得的速度",
            ...
            16: {
                title: "玩完了",
                effect() {
                    return player[this.layer].points.pow(1.9).pow(2)
                },
                effectDisplay() { return format(upgradeEffect(this.layer, this.id)) + "x" }, // Add formatting to the effect
                description: "更據能量增加超级多點數獲得速度",
        ...
        baseResource: "电力","磁力", // Name of resource prestige is based on
        baseAmount() { return player.e.points }, // Get the current amount of baseResource
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: 0.4, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        row: 1, // Row the layer is in on the tree (0 is the first row)
        hotkeys: [
            { key: "N", description: "N: Reset for never points", onPress() { if (canReset(this.layer)) doReset(this.layer) } },
        ],
        layerShown() { return true }
    }